...
def post_pr_comment(owner, repo, pull_number, token, body):
    """
    Posts a general comment to a GitHub pull request.
    """
    if not ENABLE_GITHUB_COMMENTS:
        print("GitHub PR comment publishing is disabled.")
        return 201  # Simulate success
    ...
    print(f"Posting PR comment to: {url}")

    response = requests.post(url, headers=headers, json=payload)

    print(f"GitHub PR comment status: {response.status_code}")
    if response.status_code != 201:
        print(f"GitHub PR comment response: {response.text}")
